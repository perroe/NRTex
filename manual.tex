...
The \texttt{nrdoc} class can be used both by \LaTeX\ \index{\LaTeX}
(using the \texttt{latex} command) and pdf\LaTeX\ \index{pdf\LaTeX}
...
\subsection{Closed}
The option \textit{closedavailability}
\index{option!closedavailability} gives a closed report. 

\section{Front and title page}

For the title page to work, the abstract (summary) must be
...
The date is taken from \verb!\date!.

For reports, you need to supply the isbn number. The isbn number consists
of a country code, a publisher code, a publication number and a control digit.
You can either use \texttt{isbn} to supply the full number
or \texttt{shortisbn} to supply just the last five digits:
\verb+\isbn{82-539-0481-9}+ or \verb+\shortisbn{0481-9}+.


\section{Including a figure}

On the front page, a figure\index{figure} can be included with
...
Other figures are included by \texttt{includegraphics}. An example is
...
are handled by {Bib}\TeX\ \index{cite!BiB\TeX} and 
\texttt{natbib} \cite{natbib:7.1}. By default 
an author--year citation scheme\footnote{A simpler numerical\index{cite!numeric
...
\label{tab:cite} 
\end{table}
The citations are listed in a {Bib}\TeX{} file with the extencions
...
\item[\textit{multicol}] For multiple
  columns. \index{package!multicol} 
...
\texttt{TEXINPUTS} needs to be set. If you use \texttt{tcsh} (that is
you have a \texttt{.cshrc} file), the  \texttt{.cshrc} file should
include this line:
\begin{verbatim}
setenv TEXINPUTS .:/nr/group/maler/latex:
\end{verbatim}
