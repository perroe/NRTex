...
\documentclass[report,screen,twoside,british,12pt]{nrdoc}
...
By default the year is taken from the current year at the time of processing.
The publication year\index{year} can be set using 
\texttt{publicationyear}\index{publicationyear@\verb+\publicationyear+}.
...
\end{verbatim}
If you want more than one figure on the cover you either need to combine them
using a graphics manipulation program, or use \texttt{frontpagefigurecommand}
with several 
\texttt{includegraphics}\index{includegraphics@\verb+\includegraphics+} 
commands.
